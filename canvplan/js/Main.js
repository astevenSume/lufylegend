...
var ctrlIndex = 0;
var ctrlList=[
{"frames":12,"bullet":3,"move":[-1,0],img:"enemy3",x:800,y:180,hp:100,isboss:true},
...
var bulletList = new Array(
		{startAngle:0,angle:20,step:20,speed:5,count:1},//1发
...
function setObjectEys(){
	if (frame++<10)return;	
	frame=0;
	frames++;
	var ctrlObject=ctrlList[ctrlIndex];
	if(ctrlObject && ctrlObject["frames"]==frames){
		ctrlIndex++;
		bitmapData=new LBitmapData(imglist[ctrlObject.img]);
		var enemy;		
		if (ctrlObject.isboss==true) {
...
function gameInit(result){
	...
	gameLayer.addEventListener(LEvent.ENTER_FRAME, ongameframe);
...
var mouseStartX=0,mouseStartY=0,mouseNowX=0,mouseNowY=0;
function ongameframe(event){	
	var key;
	for (key in plainLayer.childList) {
		plainLayer.childList[key].onplainframe();
	}
	var bkey;
	for (bkey in bulletLayer.childList){
		bulletLayer.childList[bkey].onbulletframe();
	}
	setObjectEys();
	showText();
	if (!player.canshoot)return;
	//console.log((player.x-player.downX)+'='+(mouseNowX-mouseStartX));
	...
	if (player.x<0){
		...
		setCoordinate(mouseNowX,mouseStartY);
	}
}
function showText(){
	hpText.text = "";
