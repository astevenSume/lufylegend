...
var yidong=function(){    
    ...
    // console.log(oevent);
    // console.log(oevent.changedTouches[0].clientX);
    var chufa=oevent.srcElement||oevent.target;
    var selfplanX=oevent.changedTouches[0].clientX;
    var selfplanY=oevent.changedTouches[0].clientY;
...
if(document.addEventListener){
    //为本方飞机添加移动和暂停
    mainDiv.addEventListener("touchstart",yidong);
    mainDiv.addEventListener("touchmove",yidong);
    //为本方飞机添加暂停事件
...
function start(){
    ...
    if(mark==20){
       ...
       // plan(hp,X,Y,sizeX,sizeY,score,dietime,sudu,boomimage,imagesrc){
        if(mark1%5==0){
            enemys.push(new enemy(6,25,274,46,60,5000,420,random(1,3),"image/zz.gif","image/enemy3_fly_1.png"));
        }
        //大飞机
        if(mark1==20){
